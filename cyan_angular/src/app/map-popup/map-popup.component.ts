...
export class MapPopupComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.compareSelected = this.locationInCompareArray(
      self.locationService.compare_locations
    );

    this.locationCompareSub = this.locationService.compare$.subscribe(
      (locations) => {
  ...
  getLocation(): void {
    ...
    self.locationSubscription = self.locationService
      .getLocationData()
      .subscribe({
        next(locations) {
          if (!self.location) {
