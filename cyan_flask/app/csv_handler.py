...
class CSVHandler:
	...
	def __init__(self):
		...
		self.csv_headers = self.location_data_headers["metaInfo"] + \
							self.location_data_headers["outputs"]  # headers for output csv
	...
	def create_rows_for_location(self, location):
		...
		row_data = self.add_location_data(location, header, meta_info)  # returns list of location rows
		return row_data
	...
	def generate_output_filename(self, input_filename):
		...
		"""
		file_components = input_filename.split(".csv")  # gets filename without extension
		if len(file_components) < 2:
			logging.warning("Batch input filename '{}' not of type .csv.".format(input_filename))
			return False
