...
class TestApiIntegration(unittest.TestCase):
	...
	base_url = "http://{}:{}/cyan/app/api/".format(os.environ.get("FLASK_HOST"), os.environ.get("FLASK_PORT"))
	...
	test_comment_obj = {
        ...
        "comment_images": [{"name": "testimage.png", "source": "base64bytestringimage"}],
        "replies": []
	}
	test_reply_obj = {
	...
	def _setup_db(self):
		...
		while attempts < max_retries:
			try:
				...
				print("Exception creating test database: {}... Retrying connection...".format(e))
				time.sleep(2)
				attempts += 1
	...
	def register_user(self):
		...
		with app_instance.test_client() as c:
			actual_response = c.post(url, json=post_data).get_json()
	...
	def login_user(self):
		...
		with app_instance.test_client() as c:
			actual_response = c.post(url, json=post_data).get_json()
			self.test_token = actual_response["user"]["auth_token"]  # sets auth token for subsequent requests
	...
	def delete_location(self):
		...
		url = "/cyan/app/api/location/delete/{}/{}".format(self.test_location_obj["id"], self.test_location_obj["type"])
		expected_response = {"status": "success"}
		with app_instance.test_client() as c:
	...
	def reset_password_update(self):
		...
		"""
		print("~~~ Running test for updating new password after password reset request.")
		token_handler = JwtHandler()
		self.test_token = token_handler.encode_auth_token(self.test_user_email).decode("utf-8")  # sets token using "email" for password update endpoint
		url = "/cyan/app/api/reset"
		post_data = {
			"email": self.test_user_email,
		...
		expected_response = {"status": "success"}
		with app_instance.test_client() as c:
			actual_response = c.put(url, json=post_data, headers=self._get_auth_headers()).get_json()
			self.assertEqual(actual_response, expected_response)
	...
	def add_comment(self):
		...
		token_handler = JwtHandler()
		self.test_token = token_handler.encode_auth_token(self.test_user).decode("utf-8")
		expected_response = dict(self.test_comment_obj)  # Update with test comment
		expected_response['comment_images'] = [expected_response['comment_images'][0]['source']]
		with app_instance.test_client() as c:
			actual_response = c.post(url, json=self.test_comment_obj, headers=self._get_auth_headers()).get_json()
			self.assertEqual(actual_response, expected_response)
	...
	def add_reply(self):
		...
		with app_instance.test_client() as c:
			actual_response = c.post(url, json=self.test_reply_obj, headers=self._get_auth_headers()).get_json()
			self.assertEqual(actual_response, expected_response)
	...
	def get_comments(self):
		...
		expected_response[0]['replies'] = [self._parse_reply_obj(dict(self.test_reply_obj))]
		expected_response[0]['comment_images'] = [self.test_comment_obj['comment_images'][0]['source']]
		with app_instance.test_client() as c:
			actual_response = c.get(url, headers=self._get_auth_headers()).get_json()
			self.assertEqual(actual_response, expected_response)
