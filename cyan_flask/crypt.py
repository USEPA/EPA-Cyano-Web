...
class CryptManager:
	def __init__(self):
		...
		self.env_location = os.path.join(
			os.path.dirname(PROJECT_ROOT), "config", "environments"
		)
		self.key_location = os.path.join(
			os.path.dirname(PROJECT_ROOT), "config", "secrets"
		)
	...
	def _open_file(self, fullpath):
		...
		if os.path.isfile(fullpath):
			logging.info("~~~ Checking for .env file: {}".format(fullpath))
			try:
				file_obj = open(fullpath, "rb")
				file_content = file_obj.read()
				file_obj.close()
				logging.info("~~~ Found .env file: {}".format(fullpath))
				return file_content
			except FileNotFoundError:
				return self._use_env_var_for_key()
		elif os.path.isfile(default_path):
			logging.info("~~~ Checking for .env file: {}".format(default_path))
			try:
				file_obj = open(default_path, "rb")
				file_content = file_obj.read()
				file_obj.close()
				logging.info("~~~ Found .env file: {}".format(default_path))
				return file_content
			except FileNotFoundError:
				return self._use_env_var_for_key()
		else:
	...
	def _get_env_as_dict(self, env_vars):
		...
		for var in env_vars:
			...
			key = var.split("=", 1)[
				0
			]  # splits at first occurence of '=', breaking up key=val pairs
			val = var.split("=", 1)[1]
	...
	def update_env_file(self, env_file, updated_env_dict):
		...
		"""
		env_vars_dict = self._open_env_file(
			env_file
		)  # opens .env file, returns dict of key:val
		print(
			"\nCurrent env vars from '{}' .env file: {}".format(env_file, env_vars_dict)
		)
		env_vars_dict.update(updated_env_dict)
		env_vars_str = self._save_env_dict_to_file(
			env_file, env_vars_dict
		)  # saves updated env vars dict to .env file
	...
	def encrypt_message(self, key_file, message, message_file=None):
		...
		if message_file:
			self._save_file(
				self.encrypted_message, message_file
			)  # saves encrypted value to file
	...
	def decrypt_message(self, key_file, message):
		...
		try:
			...
			logging.warning(
				"decrypt_message exception: {}\nReturning message without decrypting.".format(
					e
				)
			)
			return message
	...
	def rotate(self, orig_key_file, new_key_file, env_file):
		...
		for secret_key in secrets_list:
			secret_val = env_vars_dict[
				secret_key
			]  # gets current encrypted secret value
			dec_val = self.decrypt_message(
				orig_key_file, secret_val
			)  # decrypts secret with original key
			enc_val = self.encrypt_message(new_key_file, dec_val).decode(
				"utf-8"
			)  # encrypts the decrypted value with new key
		...
		updated_secrets["SK"] = self.obscure(
			Path(new_key_file).name
		)  # adds new key file path to .env file
	...
	def rotate_secrets(self, orig_key_file, env_file):
		...
		if not Path(orig_key_file).is_file():
			orig_key_file = os.path.join(
				self.key_location, orig_key_file
			)  # try default location
			if not Path(orig_key_file).is_file():
				raise Exception(
					"Secret key file '{}' does not exists.".format(orig_key_file)
				)
		...
		if Path(orig_key_file_archived).is_file():
			raise Exception(
				"Cannot save original secret key as {}. File already exist.".format(
					orig_key_file_archived
				)
			)

		orig_key = self._open_file(orig_key_file)  # orig key value
		self._save_file(
			orig_key, orig_key_file_archived
		)  # saves orig key with .orig extension
		...
		new_key = self.create_key(
			orig_key_file
		)  # overwriting orig key with newly generated key

		updated_secrets = self.rotate(
			orig_key_file_archived, orig_key_file, env_file
		)  # updates secrets and sk

		self.update_env_file(
			env_file, updated_secrets
		)  # updates .env file with secrets and sk

		print(
			"\nUpdated secret file '{}' secret values: {}".format(
				env_file, updated_secrets
			)
		)
...
if __name__ == "__main__":
	...
	if method == "encrypt":
		...
		try:
			...
			save_file = input(
				"Enter path and filename for storing encrypted secret (e.g., /config/secrets/enc.txt): "
			)
		...
		message = sys.argv[
			3
		]  # comma-separated list in quotes (e.g., "file1.txt, file2.txt")
